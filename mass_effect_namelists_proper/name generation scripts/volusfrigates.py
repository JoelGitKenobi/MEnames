...
FORMAL_SUFFIXES = [
    "akar", "alen", "anor", "aran", "aren", "aros", "avar", "avor", "edan",
    "edor", "ekar", "elan", "elor", "enar", "evar", "ikan", "ikor", "inar",
    "inos", "ivar", "oran", "oren", "orik", "ovar", "ular", "ulen", "unor",
    "vakar", "venor", "vesh", "yanor",
]

PAIR_ENDINGS = [
    "ban", "cal", "dao", "din", "for", "ker", "kur", "mar", "non", "sab",
    "ser", "shol", "tor", "tun", "varr", "wen", "yap",
]

INSERTS = ["r", "l", "n", "v", "s", "th", "sh", "sk", "vr", "lk", "nd", "rn"]
...
def build_fragments(roots: list[str]) -> tuple[list[str], list[str], list[str]]:
    starts: set[str] = set()
    middles: set[str] = set()
    ends: set[str] = set(FORMAL_SUFFIXES)
    for root in roots:
...
def volify_foreign_root(root: str, rng: random.Random) -> str:
    ...
    word += rng.choice(FORMAL_SUFFIXES)
    return clean_single(word)


def build_formal_native(roots: list[str], rng: random.Random) -> str:
    base = normalize_word(rng.choice(roots))
    if len(base) > 5 and rng.random() < 0.7:
        base = base[:-1]
    if rng.random() < 0.35:
        insert_at = rng.randint(1, max(1, len(base) - 1))
        base = base[:insert_at] + rng.choice(INSERTS) + base[insert_at:]
    base += rng.choice(FORMAL_SUFFIXES)
    return clean_single(base)


def build_honor_blend(roots: list[str], rng: random.Random) -> str:
    ...
    return clean_single(left_cut + right_cut + rng.choice(["ar", "or", "en", "ik", "un"]))


def build_fragment_name(starts: list[str], middles: list[str], ends: list[str], rng: random.Random) -> str:
    raw = rng.choice(starts)
    for _ in range(rng.choices([1, 2, 3], weights=[24, 48, 28], k=1)[0]):
        raw += rng.choice(middles)
    raw += rng.choice(ends)
    return clean_single(raw)


def build_lineage_pair(roots: list[str], rng: random.Random) -> str:
    left = normalize_word(rng.choice(roots))
    right = normalize_word(rng.choice(roots))
    if len(left) > 5:
        left = left[: rng.randint(2, 4)]
    if len(right) > 5:
        right = right[-rng.randint(2, 4):]
    return title_case(f"{left} {right}{rng.choice(PAIR_ENDINGS)}")
...
def too_similar(candidate: str, used: set[str]) -> bool:
    ...
    for prior in used:
        ...
        if len(key) >= 7 and len(prior) >= 7 and key[:4] == prior[:4]:
            return True
        if SequenceMatcher(None, key, prior).ratio() >= 0.89:
            return True
    return False
...
def generate_names(
    ...
    while len(results) < count and attempts < max_attempts:
        attempts += 1
        mode = rng.choices(
            population=["formal_native", "honor_blend", "fragment", "borrowed", "lineage_pair"],
            weights=[30, 22, 18, 20, 10],
            k=1,
        )[0]

        if mode == "formal_native":
            candidate = build_formal_native(native_roots, rng)
        elif mode == "honor_blend":
            candidate = build_honor_blend(native_roots, rng)
        elif mode == "fragment":
            candidate = build_fragment_name(starts, middles, ends, rng)
        elif mode == "borrowed":
            candidate = volify_foreign_root(rng.choice(foreign_roots), rng)
        else:
            candidate = build_lineage_pair(native_roots, rng)
    ...
    if len(results) < count:
        raise RuntimeError(f"Could only generate {len(results)} unique Volus frigate names after {attempts} attempts.")
    return results
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Volus frigate names as formal native honor-names with a limited protectorate-borrowed layer "
            "adapted from Turian roots rather than direct copies."
        )
    )
